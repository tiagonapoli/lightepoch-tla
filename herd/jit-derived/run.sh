...
note ""
note "--- Hazard 3c: the user-word column across a slot handoff ---"
note "# User words share the Entry cache line with localCurrentEpoch and threadId and"
note "# are deliberately NOT reset on Acquire/Release, so the next owner of a slot"
note "# inherits whatever the previous owner left. TsavoriteLog depends on that: it"
note "# publishes each in-flight enqueue into its user word and folds the column into"
note "# SafeTailAddress. The question is whether the departing owner's InflightInactive"
note "# is visible to the thread that claims the slot next."
run x86-userword-handoff-main    Never     "x86-TSO already orders the user-word write before the plain handover store"
run x86-userword-handoff-fixed   Never     "unchanged on x86: the fix neither supplies nor removes ordering TSO already gave"
run arm64-userword-handoff-main  Sometimes "upstream hands the slot over with a plain store, so the sentinel can sink past it"
run arm64-userword-handoff-fixed Never     "the release store on localCurrentEpoch pairs with the claim CAS and carries the sentinel"

note ""
note "--- Hazard 3d: the user word the OTHER way -- a lost in-flight publish ---"
note "# 3c asks whether the arriving owner can read a stale in-flight address; that"
note "# only makes SafeTailAddress lag. This asks whether the DEPARTING owner's"
note "# sentinel can land on top of the arriving owner's live publish, which erases an"
note "# in-flight enqueue and lets SafeTailAddress advance past a slot whose payload"
note "# has not been written. That is a torn read, not a stall."
run x86-userword-lostpublish-main    Never     "TSO orders the departing owner's sentinel before the handover, so it cannot land last"
run arm64-userword-lostpublish-main  Sometimes "with a plain handover the sentinel can overtake the new owner's publish"
run arm64-userword-lostpublish-fixed Never     "the release store keeps the sentinel before the claim the new owner's publish follows"
