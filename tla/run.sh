...
echo ""
echo "--- the same query on a CONTENDED TABLE, with TryAcquireEntry's probe window ---"
echo "# The rows above use one slot. That is enough for the false NEGATIVE, but it"
echo "# cannot express the false POSITIVE at all, so the spec above only argues that"
echo "# direction. TryAcquireEntry writes the candidate index into the thread-private"
echo "# Metadata.Entries[instanceId] BEFORE the claim can fail, so between the first"
echo "# candidate and a successful claim the thread has a NON-INVALID entry index"
echo "# pointing at a slot another thread owns. With one slot there is nothing to"
echo "# probe past and that window has no states."
echo "#"
echo "# Two slots and two readers is the smallest table that forces a reader to walk"
echo "# past a slot another reader holds. On it, both directions are CHECKED, and"
echo "# NoFalsePositive is checked in EVERY state rather than at a program point --"
echo "# ~130 call sites read this query, four of them from I/O completion callbacks,"
echo "# so 'accurate wherever it is asked' is the property worth having."
run "$HERE/epoch/fixes" CasAnnounceProtectedQueryProbe CasAnnounceProtectedQueryProbe_tagged_release_tso.cfg HOLDS "THE X86 RESULT: the fix answers correctly in both directions on a contended table"
run "$HERE/epoch/fixes" CasAnnounceProtectedQueryProbe CasAnnounceProtectedQueryProbe_tagged_release_arm.cfg HOLDS "same, also robust to StoreStore reordering"
echo "# What threadId is still FOR, now that it no longer gates entry to the slot."
echo "# Drop the tag and keep everything else and the probe window immediately"
echo "# reports protection over a slot the thread does not hold. This is the row that"
echo "# says the tag cannot simply be deleted along with its CAS."
run "$HERE/epoch/fixes" CasAnnounceProtectedQueryProbe CasAnnounceProtectedQueryProbe_indexonly_tso.cfg VIOLATED "control (tag axis): without the tag, probing IS a false positive"
echo "# The ORDER axis again, and again it bites on plain x86 -- but now in BOTH"
echo "# directions. Upstream Release() publishes the slot free while this thread's tag"
echo "# still matches and its entry index is still valid, so for that window the"
echo "# thread claims protection over a slot another thread may already have taken."
run "$HERE/epoch/fixes" CasAnnounceProtectedQueryProbe CasAnnounceProtectedQueryProbe_tagged_upstream_fp_tso.cfg VIOLATED "control (order axis): upstream Release() yields a false POSITIVE on plain x86"
run "$HERE/epoch/fixes" CasAnnounceProtectedQueryProbe CasAnnounceProtectedQueryProbe_tagged_upstream_fn_tso.cfg VIOLATED "control (order axis): and a false negative, on plain x86"
echo "# The FENCE axis is ARM-only, as everywhere else, and it is one-sided: a plain"
echo "# unpublish can lose the next owner's tag, but it cannot manufacture a tag that"
echo "# matches the asking thread, because only an owner ever writes its own id."
run "$HERE/epoch/fixes" CasAnnounceProtectedQueryProbe CasAnnounceProtectedQueryProbe_tagged_plain_fp_arm.cfg HOLDS    "a plain unpublish cannot manufacture a false positive"
run "$HERE/epoch/fixes" CasAnnounceProtectedQueryProbe CasAnnounceProtectedQueryProbe_tagged_plain_fn_arm.cfg VIOLATED "control (fence axis): but it does wipe the next owner's tag (+StoreStore)"
...
echo ""
echo "========= Stale announce: reclamation \"going back in time\" ========="
echo "# Acquire() reads CurrentEpoch and then probes for a free slot, so the value it"
echo "# announces may already be behind CurrentEpoch when the CAS lands. Raised in"
echo "# review: can a thread announce an epoch the reclaimer has already declared safe,"
echo "# and then dereference something that was freed on the strength of that decision?"
echo "# The coverage row proves the interleaving is reached; the safety row proves it"
echo "# is harmless, because a locked-RMW announce is a full barrier and so forces the"
echo "# entering thread to observe the unlink that preceded the free."
run "$HERE/epoch/fixes" CasAnnounceStaleEpoch  CasAnnounceStaleEpoch_coverage_tso.cfg   VIOLATED "the free-then-stale-announce interleaving really is reachable"
run "$HERE/epoch/fixes" CasAnnounceStaleEpoch  CasAnnounceStaleEpoch_cas_tso.cfg        HOLDS    "yet no use-after-free: the RMW announce orders the dereference after the unlink"
run "$HERE/epoch/fixes" CasAnnounceStaleEpoch  CasAnnounceStaleEpoch_cas_arm.cfg        HOLDS    "and it still holds with StoreStore relaxed"
run "$HERE/epoch/fixes" CasAnnounceStaleEpoch  CasAnnounceStaleEpoch_plain_tso.cfg      VIOLATED "the baseline plain-store announce does NOT survive it, so the check is alive"
echo "# Accounting, not safety: a stale announce leaves SafeToReclaimEpoch transiently"
echo "# at or above a live announcement, i.e. it is not monotonic. Benign -- a lower"
echo "# safe epoch only delays future frees and fired drain entries never re-fire --"
echo "# and the unfixed baseline has the same property."
run "$HERE/epoch/fixes" CasAnnounceStaleEpoch  CasAnnounceStaleEpoch_accounting_tso.cfg VIOLATED "SafeToReclaimEpoch is not monotonic under a stale announce"
