...
echo ""
echo "--- what the threadId CAS was protecting: TrySuspend / ResumeIfNotProtected ---"
echo "# The fix moves the claim off threadId, so threadId stops being an ownership"
echo "# word and becomes a plain store that trails the claim. ThisInstanceProtected()"
echo "# still reads it, and TrySuspend() and ResumeIfNotProtected() still branch on"
echo "# the answer. These rows make the thread PERFORM that load and act on it."
echo "#"
echo "# A false positive is unreachable by construction -- the query also tests the"
echo "# thread-private entry index -- so the hazard is the false negative, and each"
echo "# API turns it into a different failure: TrySuspend returns false and never"
echo "# suspends (the announce pins SafeToReclaimEpoch forever), ResumeIfNotProtected"
echo "# returns true and acquires a SECOND slot, orphaning the first."
run "$HERE/epoch/fixes" CasAnnounceProtectedQuery CasAnnounceProtectedQuery_release_tso.cfg  HOLDS    "the fix: clear threadId, then unpublish with a release store (x86-TSO)"
run "$HERE/epoch/fixes" CasAnnounceProtectedQuery CasAnnounceProtectedQuery_release_arm.cfg  HOLDS    "the fix: clear threadId, then unpublish with a release store (+StoreStore)"
echo "# Two controls on two independent axes. The FENCE axis, as elsewhere: TSO's"
echo "# FIFO drain already orders the pair, so only a StoreStore-relaxing model bites."
run "$HERE/epoch/fixes" CasAnnounceProtectedQuery CasAnnounceProtectedQuery_plain_tso.cfg    HOLDS    "control (fence axis): FIFO drain orders the clear before the unpublish"
run "$HERE/epoch/fixes" CasAnnounceProtectedQuery CasAnnounceProtectedQuery_plain_arm.cfg    VIOLATED "control (fence axis): the unpublish overtakes the clear and wipes the next owner's tag"
echo "# The ORDER axis, which is new and is the one that matters for an x86-only"
echo "# change: keep the fix's CAS but leave Release() in its upstream order and the"
echo "# query breaks under plain TSO, no weak memory required. Inverting those two"
echo "# stores is load-bearing on x86, not a weak-memory nicety."
run "$HERE/epoch/fixes" CasAnnounceProtectedQuery CasAnnounceProtectedQuery_upstream_tso.cfg VIOLATED "control (order axis): upstream Release() order breaks the query on plain x86"
run "$HERE/epoch/fixes" CasAnnounceProtectedQuery CasAnnounceProtectedQuery_upstream_arm.cfg VIOLATED "control (order axis): upstream Release() order breaks the query (+StoreStore)"
