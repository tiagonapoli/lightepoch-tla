------------------------------ MODULE FixedLightEpochWithInterlocked ------------------------------
(***************************************************************************)
(* FixedLightEpochWithInterlocked — announce via an atomic seq-cst RMW.    *)
(*                                                                         *)
...
\* Reader ---------------------------------------------------------------------
\* Announce via Interlocked.Exchange: an atomic seq-cst RMW publishes the
\* announce AND carries a full barrier, so it drains the reader's buffer and
\* closes the StoreLoad window exactly like the explicit full barrier.
Acq ==
    /\ pcRd = "acq"
    /\ eRead' = Load(Rd, "ce")
