----------------------- MODULE CasAnnounceNoThreadIdNoCas -----------------------
(***************************************************************************)
(* SECOND CONTROL for CasAnnounceNoThreadId.tla, on the other axis.         *)
(*                                                                         *)
(* CasAnnounceNoThreadIdStaleIndex.tla proves the model can see an          *)
(* OWNERSHIP failure. This one proves it can still see an ANNOUNCE          *)
(* failure -- the original LightEpoch bug -- after threadId is deleted.     *)
(* Identical to CasAnnounceNoThreadId.tla except that Claim tests and sets  *)
(* the slot with an ordinary read and an ordinary buffered store instead of *)
(* a locked RMW.                                                            *)
(*                                                                         *)
(* Without this, a HOLDS from CasAnnounceNoThreadId could mean only that    *)
(* removing threadId removed the invariants that had teeth.                 *)
(*                                                                         *)
(* Expected: VIOLATED under both "tso" and "arm".                           *)
(***************************************************************************)
(*                                                                         *)
(* Original header, retained because everything else is unchanged:          *)
(*                                                                         *)
...
\* The fix removed. The slot is tested with an ordinary load and taken with an
\* ordinary buffered store, so the announce can sit in this core's store buffer
\* while the reclaimer's scan reads the slot as empty.
Claim(r) ==
    /\ readerPc[r] = "Claim"
