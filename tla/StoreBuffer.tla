...
(* VARIABLES mem, sb and CONSTANT Model simply writes:                     *)
(*                                                                         *)
(*     SB == INSTANCE StoreBuffer                                          *)
(*                                                                         *)
(* and then uses SB!Load, SB!Buffer, SB!Fenced, SB!Drained, SB!FlushOne.    *)
(* SB!FlushOne constrains only mem' and sb', so the caller conjoins its own *)
(* UNCHANGED <<...>> for the remaining variables.                          *)
(*                                                                         *)
(* STATE SHAPE                                                             *)
(*   mem    a record of shared fields, e.g.                                *)
(*            [ ce |-> 1, lce |-> 0, ret |-> FALSE, freed |-> FALSE ]      *)
(*   sb     sb[p] is processor p's private FIFO of pending stores, each a  *)
(*          record [f |-> <field of mem>, v |-> <value>]                   *)
(*                                                                         *)
...
(* domains, and multi-copy atomicity (there is a single shared `mem`, so    *)
...
Buffer(p, f, v) == [sb EXCEPT ![p] = Append(sb[p], [f |-> f, v |-> v])]
...
(*     mem' = SB!Fenced(p)   /\   sb' = SB!Drained(p)                      *)
(***************************************************************************)
Fenced(p)  == ApplyAll(mem, sb[p])
Drained(p) == [sb EXCEPT ![p] = <<>>]
...
(*     mem' = SB!FencedStore(p, "lce", e)   /\   sb' = SB!Drained(p)       *)
(***************************************************************************)
FencedStore(p, f, v) == ApplyAll(mem, Append(sb[p], [f |-> f, v |-> v]))

(***************************************************************************)
(* Asynchronous propagation: one pending store becomes globally visible.    *)
(* Constrains mem' and sb' only — conjoin UNCHANGED for the rest.          *)
(***************************************************************************)
FlushOne(p) ==
    /\ sb[p] # <<>>
    /\ IF Model = "tso"
       THEN /\ mem' = [mem EXCEPT ![Head(sb[p]).f] = Head(sb[p]).v]
            /\ sb'  = [sb EXCEPT ![p] = Tail(sb[p])]
       ELSE \E i \in DOMAIN sb[p] :
              /\ mem' = [mem EXCEPT ![sb[p][i].f] = sb[p][i].v]
              /\ sb'  = [sb EXCEPT ![p] =
                           SubSeq(sb[p], 1, i-1) \o SubSeq(sb[p], i+1, Len(sb[p]))]
=============================================================================
